  ...
  if (isMobile) {
    return (
      <div className="absolute inset-0 z-30">
        ...
        >
          <div
            className={`flex flex-shrink-0 items-center border-b border-white/10 px-5 py-4 ${bitsMode ? 'justify-end' : 'justify-between'}`}
          >
            {!bitsMode ? (
  ...
  return (
    <div className="absolute inset-0 z-30 pointer-events-none">
      ...
      <aside
        ...
        aria-modal="true"
        aria-label={dialogAriaLabel}
        className={`pointer-events-auto absolute right-6 top-6 bottom-6 flex flex-col overflow-hidden rounded-[28px] border border-white/10 bg-[#071225]/70 shadow-[0_18px_60px_rgba(0,0,0,0.55)] backdrop-blur-xl ${mode === 'bits' ? 'w-[min(calc(100vw-3rem),400px)]' : 'w-[360px]'}`}
        style={{
